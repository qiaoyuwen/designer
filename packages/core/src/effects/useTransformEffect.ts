...
import { DragStartEvent, DragMoveEvent, DragStopEvent, KeyDownEvent, KeyUpEvent, ViewportScrollEvent } from '../events';
import { ICustomEvent, KeyCode } from '@designer/utils';
...
export const useTransformEffect = (engine: Engine) => {
  ...
  const getResizeHandler = (target: HTMLElement) => {
    const handler = target?.closest(`*[${engine.props.nodeResizeHandlerAttrName}]`) as any;
    if (!handler) return;
  ...
  const getRotateHandler = (target: HTMLElement) => {
    const handler = target?.closest(`*[${engine.props.nodeRotateHandlerAttrName}]`) as any;
    if (!handler) return;
    return { handler };
  };
