...
import { uid, ICustomEvent, EventContainer } from '@designer/utils';
...
export class Workspace {
  ...
  public outline: Viewport;

  public operation: Operation;

  public history: History<Workspace>;

  public props: IWorkspaceProps;

  public constructor(engine: Engine, props: IWorkspaceProps) {
    this.engine = engine;
    this.props = props;
  ...
  public attachEvents(container: EventContainer, contentWindow: Window) {
  ...
  public dispatch(event: ICustomEvent) {
    return this.engine.dispatch(event, this.getEventContext());
  }

  public serialize(): IWorkspace {
    return {
      id: this.id,
      title: this.title,
  ...
  public from(workspace?: IWorkspace) {
    if (!workspace) return;
