...
export class LineSegment {
  public start: IPoint;
  public end: IPoint;
  public constructor(start: IPoint, end: IPoint) {
    this.start = {
      ...start,
      x: start.x,
      y: start.y,
...
export class Vector {
  public x: number;
  public y: number;
  public constructor(line: ILineSegment) {
...
export function isPointInRect(point: IPoint, rect: IRect, sensitive: boolean = true) {
...
export function isEqualLineSegment(target: ILineSegment, source: ILineSegment) {
  return isEqualPoint(target?.start, source?.start) && isEqualPoint(target?.end, source?.end);
}
...
export function calcIncludedAngleOfLine(line1: ILineSegment, line2: ILineSegment) {
...
export function isNearAfter(point: IPoint, rect: IRect, inline: boolean = false) {
  if (inline) {
    return (
...
export function calcClosestEdges(
  ...
  const h_types = ['ht', 'hc', 'hb'];
  const v_types = ['vl', 'vc', 'vr'];
  let edge: string;
  ...
  if (line?.start?.y === line?.end?.y) {
    edges.h.forEach((target, index) => {
      const _offset = precision(Math.round(line.start.y) - Math.round(target.start.y), 2);
    ...
    edges.v.forEach((target, index) => {
      const _offset = precision(Math.round(line.start.x) - Math.round(target.start.x), 2);
      const _distance = Math.abs(_offset);
      if (_distance < distance) {
        distance = _distance;
        offset = _offset;
