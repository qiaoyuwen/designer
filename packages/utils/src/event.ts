...
export class EventDriver<Engine extends Event = Event, Context = any> implements IEventDriver {
  ...
  public contentWindow: Window = globalThisPolyfill;
  ...
  public dispatch<T extends ICustomEvent<any> = any>(event: T) {
    return this.engine.dispatch(event, this.context);
  }

  public subscribe<T extends ICustomEvent<any> = any>(subscriber: ISubscriber<T>) {
...
export class Event extends Subscribable<ICustomEvent<any>> {
  ...
  public attachEvents(container: EventContainer, contentWindow: Window = globalThisPolyfill, context?: any) {
    if (!container) return;
