...
import { isResourceHost, isResourceList, IResourceLike, IResource } from '@designer/core';
import { isFn } from '@designer/utils';
    ...
    return (
      ...
      >
        ...
        {icon && React.isValidElement(icon) ? (
          ...
          <IconWidget className={prefix + '-item-icon'} infer={icon} style={{ width: 150, height: 40 }} />
        )}
    ...
    >
      ...
      <div className={prefix + '-content-wrapper'}>
        <div className={prefix + '-content'}>
          ...
          {remainItems ? (
            <div className={prefix + '-item-remain'} style={{ gridColumnStart: `span ${3 - remainItems}` }}></div>
          ) : null}
        </div>
      </div>
    </div>
