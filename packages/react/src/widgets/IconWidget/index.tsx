...
import { isStr, isFn, isObj, isPlainObj } from '@designer/utils';
...
const IconContext = createContext<IIconProviderProps>(null);
...
export const IconWidget: React.FC<IIconWidgetProps> & {
  Provider?: React.FC<PropsWithChildren<IIconProviderProps>>;
  ShadowSVG?: React.FC<PropsWithChildren<IShadowSVGProps>>;
  ...
  const renderTooltips = (children: React.ReactElement): React.ReactElement => {
    ...
    if (tooltip) {
      const title = React.isValidElement(tooltip) || isStr(tooltip) ? tooltip : (tooltip as any).title;
